...
const createAuthor = async function (req, res) {
  try {
    ...
    if(!isValidTitle(data.title)) return res.status(400).send({status:false, msg:"Please Enter Title"})
...
const authorLogin=async function (req, res){
  try{
    ...
    if (!/^\w+([\.-]?\w+)@\w+([\.-]?\w+)(\.\w{2,3})+$/.test(requestBody.email)) return res.status(400).send({ status:false, message: "Pls Enter Email in valid Format" })
